...
import requests
from bitexen_client import version
...
class APIException(Exception):
    ...
    def __str__(self):
        return repr(f"{self.code}: {self.value}")
...
class API:
    ...
    def __init__(
        ...
        self.session.headers.update(
            {"User-Agent": f"bitexen-python/{version.__version__} (+{version.__url__})"}
        )
        self.response = None
        self._json_options = {}
    ...
    def _query(self, urlpath, data, auth=None, timeout=None):
        ...
        if data == {}:
            self.response = self.session.get(url, auth=auth, timeout=timeout)
        else:
            self.response = self.session.post(url, json=data, auth=auth, timeout=timeout)
    ...
    def _get_orders(self, account_name="Main", status="", market_code=""):
        method = f"orders/{account_name}/{status}/{market_code}/"
    ...
    def get_market_info(self, market_code=""):
        method = f"market_info/{market_code}/"
    ...
    def cancel_order(self, order_number):
        method = f"cancel_order/{order_number}/"
    ...
    def get_order_status(self, order_number):
        method = f"order_status/{order_number}/"
        result = dotdict(self._query_private(method, timeout=self.timeout))
    ...
    def get_order_book(self, market_code):
        method = f"order_book/{market_code}/"
        result = dotdict(self._query_public(method, timeout=self.timeout))
